...
心跳包记录在ft\_online\_client（精确到分钟）和ft\_online\_client\_hour（每小时一条）表中。小时是分钟统计的总和，只要这个小时内有一个心跳包，就算这个小时内有心跳包。日统计统计ft\_online\_client\_hour中一天中的24条记录，只要有一条记录表示当天有心跳包。所以，只要一天内有一个心跳包，就算这个用户在这个设备当天是活跃的。

活跃度：与心跳包正相关。日活跃度暂定为小时中的记录数，月活跃度暂定为日活跃度之和（先不做）
...
| account1 | account | 日 | 日活跃用户总数 | 一天 | 每天3:00:00 | 日活跃用户总数。统计时是统计前一天的数据，下同。例如4月11号是统计4月10号的数据 |
| account2 | account | 日 | touch日活跃用户数 | 一天 | 每天 3：00:00 | 日活跃用户：登录过。有过心跳包 |
| account3 | account | 日 | android日活跃用户数 | 一天 | 每天 3：00:00 | android日活跃用户：登录过。有过心跳包 |
| account4 | account | 日 | ios日活跃用户数 | 一天 | 每天 3：00:00 | ios日活跃用户：登录过。有过心跳包 |
| account5 | account | 月 | 月活跃用户总数 | 当天 | 每天3:00:00 |  |
| account6 | account | 月 | touch日活跃用户数 | 一天 | 每天 3：00:00 | 日活跃用户：登录过。有过心跳包 |
| account7 | account | 月 | android日活跃用户数 | 一天 | 每天 3：00:00 | android日活跃用户：登录过。有过心跳包 |
| account8 | account | 月 | ios日活跃用户数 | 一天 | 每天 3：00:00 | ios日活跃用户：登录过。有过心跳包 |
